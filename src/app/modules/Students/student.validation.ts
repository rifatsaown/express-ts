...
const studentValidationSchema = z.object({
  ...
  gender: z.enum(['male', 'female'], {
    errorMap: () => ({ message: `Gender is required and should be either male or female` }),
  }),
  dateOfBirth: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Invalid date of birth'),
  age: z.number().nonnegative('Age must be a non-negative number').min(1, 'Age must be at least 1'),
  email: z.string().email('Invalid email address'),
  phone: z.string().regex(/^\+?[0-9]{10,}$/, 'Invalid phone number'),
  emergencyContact: z.string().regex(/^(\+88)?01[0-9]{9}$/, 'Invalid phone number'),
